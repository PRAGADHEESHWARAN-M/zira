...
export default function Cart({ cart, setCart }) {
  ...
  return (
    <div style={{ maxWidth: 700, margin: "0 auto", padding: "40px 20px" }}>
      ...
      {cart.length === 0 && <p style={{ color: "#8fa3b5" }}>Your bag is empty.</p>}
      {cart.map((l) => (
        <div key={l.product._id} className="card" style={{ display: "flex", alignItems: "center", gap: 16, padding: 16, marginBottom: 12 }}>
          <div style={{ fontSize: 26, color: "#4a90e2" }}>{l.product.icon || "◆"}</div>
          <div style={{ flex: 1 }}>
            <p style={{ fontSize: 14 }}>{l.product.name}</p>
            <p style={{ fontSize: 12, color: "#8fa3b5" }}>{currency(l.product.price)} each</p>
          </div>
          <input
          ...
          />
          <Trash2 size={16} style={{ cursor: "pointer", color: "#8fa3b5" }} onClick={() => remove(l.product._id)} />
        </div>
      ))}
      ...
      {cart.length > 0 && (
        <div style={{ textAlign: "right", marginTop: 20 }}>
          <p className="serif" style={{ fontSize: 22, marginBottom: 14 }}>
            Total: <span style={{ color: "#87ceeb" }}>{currency(total)}</span>
          </p>
